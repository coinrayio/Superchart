...
export interface ScriptEditorProps {
  ...
  /**
   * Open the editor in read-only mode (e.g. when viewing a server preset indicator's code).
   * A "Clone & Edit" button replaces "Add to Chart" so the user can fork it.
   */
  readOnly?: boolean
  /** Called when user clicks "Clone & Edit" in read-only mode */
      ...
      <div className="superchart-se-modal-content">
        ...
        <div className="superchart-se-toolbar">
          ...
          <div className="superchart-se-toolbar-right">
            ...
            {/* Read-only: Clone & Edit button */}
            {readOnly && (
              <button
                className="superchart-se-primary-btn"
                onClick={() => {
                  setReadOnly(false)
                  onCloneAndEdit?.(code)
                }}
              >
                <svg width="16" height="16" viewBox="0 0 16 16" style={{ marginRight: '6px' }}>
                  <path d="M2 14l1-4L11 2l3 3-8 8-4 1zm9-10l1 1" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                <span>{i18n('clone_and_edit', locale) || 'Clone & Edit'}</span>
              </button>
            )}

            {/* Add/Update on chart button (hidden in read-only mode) */}
            {!readOnly && onAddToChart && (
              <button
        ...
        {/* Read-only banner */}
        {readOnly && (
          <div className="superchart-se-readonly-banner">
            View only — click <strong>Clone &amp; Edit</strong> to make your own copy
          </div>
        )}

        {/* Editor area */}
        <div className={`superchart-se-editor-area${readOnly ? ' superchart-se-editor-readonly' : ''}`}>
          ...
          {cmAvailable === false && (
            <div className="superchart-se-textarea-wrapper">
              ...
              <textarea
                ...
                readOnly={readOnly}
                spellCheck={false}
                style={{ fontSize: `${fontSize}px`, opacity: readOnly ? 0.75 : 1 }}
              />
            </div>
          )}
        </div>
